...
TreatmentSession::TreatmentSession(EEGInterface &eegInterface,
...
void TreatmentSession::applyTreatmentToSite(double baselineFrequency,
                                            int siteIndex) {
    const double offsetFrequency = 5.0;  // Hz
    double currentFrequency = baselineFrequency;
    for (int i = 0; i < 16;
         ++i) {  // Apply treatment every 1/16th of a second for 1 second
        currentFrequency += offsetFrequency;
        // Simulate recalculating the brainwave frequency and applying the
        // offset
        std::this_thread::sleep_for(
            std::chrono::milliseconds(62));  // Simulate time delay
    }
...
void TreatmentSession::exportSessionData() const {
    ...
    sessionLog << "Initial Overall Baseline Frequency: "
               << overallBaselineFrequency << "\n";
    for (int i = 0; i < static_cast<int>(siteBaselineFrequencies.size()); i++) {
        sessionLog << "Site " << i << " Baseline Frequency (Before): "
                   << siteBaselineFrequencies[i] << "\n";
    }
    sessionLog << "Final Overall Baseline Frequency: "
               << overallBaselineFrequency << "\n";
    for (int i = 0; i < static_cast<int>(siteBaselineFrequencies.size()); i++) {
        sessionLog << "Site " << i << " Baseline Frequency (After): "
                   << siteBaselineFrequencies[i] << "\n";
    }
    sessionLog << "\n";
    sessionLog.close();
...
double TreatmentSession::calculateDominantFrequency() {
    ...
    double fd = (f1 * A1 * A1 + f2 * A2 * A2 + f3 * A3 * A3) /
                (A1 * A1 + A2 * A2 + A3 * A3);
